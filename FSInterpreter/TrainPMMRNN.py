...
"""
Script for training the RNN used by the Phrase Model Manager.
Minor changes to the PTB LSTM model example from Google's TensorFlow.
==========
...
$ python TrainPMMRNN.py <model_size>
...
def process_args():
    ...
    """
    parser = argparse.ArgumentParser(description='Train Phrase Model Manager RNN (update model variables).')
    parser.add_argument('model_size', type=str, help='config size for model (small, medium, or large)')
...
def main():
    ...
    raw_data = reader.ptb_raw_data(DATA_DIRECTORY)
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        saver.save(session, MODEL_DIRECTORY + 'train-vars-rnn', write_meta_graph=False)
